...
use nomo_lockfile::{DependencyGraph, ResolvedDependency, filter_dependency_graph_for_target};
...
pub fn dependency_tree_for_target_with_options(
    project: &Project,
    options: DependencyResolutionOptions,
    target: &TargetTriple,
) -> Result<String, String> {
    ...
    Ok(render_dependency_tree(&filter_dependency_graph_for_target(
        &graph, target,
    )))
}
...
fn target_suffix(dependency: &ResolvedDependency) -> String {
    if dependency.target.is_unconditional() {
        String::new()
    } else {
        format!(" [{}]", dependency.target)
    }
}

...
pub(super) fn emit_expr(out: &mut String, expr: &ValueExpr) {
    ...
    if emit_string_char_expr(out, expr) {
        return;
    }
    if emit_std_misc_expr(out, expr) {
        return;
    }
    if emit_collections_expr(out, expr) {
        return;
    }
    ...
    match expr {
        ...
        | ValueExpr::CharToString { .. } => {
            unreachable!("string/char expressions are emitted before the main expression match")
        }
        ...
        | ValueExpr::RegexCaptures { .. } => {
            unreachable!("std misc expressions are emitted before the main expression match")
        }
        ...
        | ValueExpr::CollectionsStringSetRemove { .. } => {
            unreachable!("collections expressions are emitted before the main expression match")
        }

...
pub(super) fn uses_async_tcp_io(program: &Program) -> bool {
    program.functions.iter().any(|function| {
        function.body.iter().any(|statement| {
            statement_contains_expr(statement, |expr| {
                matches!(
...
pub(super) fn uses_jsonrpc_builtin(program: &Program) -> bool {
    program.functions.iter().any(|function| {
        function
            .body
            .iter()
            .any(|statement| statement_contains_expr(statement, expr_is_jsonrpc_builtin))
    })
}
...
pub(super) fn uses_sqlite_runtime(program: &Program) -> bool {
    program.functions.iter().any(|function| {
        function
            .body
            .iter()
            .any(|statement| statement_contains_expr(statement, expr_is_sqlite_call))
    })
}

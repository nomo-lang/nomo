...
pub(super) fn lower_struct_literal_value_expr(
    ...
    if is_task_runtime_opaque_struct(struct_type) {
        return Err(Diagnostic::new(
            "E0820",
            format!("runtime-owned task type `{type_name}` cannot be constructed in Nomo"),
            path,
            span.line,
            span.column,
            span.length,
    ...
    if is_jsonrpc_opaque_struct(struct_type) {
        return Err(Diagnostic::new(
            "E0840",
            format!("opaque JSON-RPC type `{type_name}` cannot be constructed in Nomo"),
            path,
            span.line,
            span.column,
            span.length,
    ...
    if is_cron_opaque_struct(struct_type) {
        return Err(Diagnostic::new(
            "E0850",
            format!("opaque cron type `{type_name}` cannot be constructed in Nomo"),
            path,
            span.line,
            span.column,
            span.length,

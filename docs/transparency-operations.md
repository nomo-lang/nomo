...
Nomo's `signed+transparent` policy treats the project-config-pinned Ed25519 key
as a root of trust, not as a key that must sign every future tree head.
Production logs can rotate that key, exchange signed checkpoints between
clients, and enforce explicit online and offline proof ages without weakening
the original pin.
...
1. The first old key must match a key pinned in
   `registry.transparency-keys`.
...
channel used for the original project-config pin. Clients should not auto-trust an
...
Projects can list checkpoint files in `.nomo/config.toml`:
...
Each file may contain one checkpoint or a JSON array. Paths are project- or
workspace-root-relative and cannot escape that root. After a resolver
verification, Nomo writes

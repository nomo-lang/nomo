...
fn run() -> Result<(), String> {
    ...
    match command.as_str() {
        ...
        "build" => {
            let (source, out) = parse_build_args(args)?;
            let c = compile_source_to_c(&source).map_err(|diag| diag.human())?;
            if let Some(out) = out {
...
fn parse_build_args(args: Vec<String>) -> Result<(PathBuf, Option<PathBuf>), String> {
    let mut source = None;
    let mut out = None;
        ...
        match arg.as_str() {
            ...
            "--out" => {
                let Some(value) = iter.next() else {
                    return Err(
                        "usage: nomoc build <source.nomo> [--emit-c] [--out path]".to_string()
                    );
                };
                out = Some(PathBuf::from(value));
            }
            _ if source.is_none() => source = Some(PathBuf::from(arg)),
            _ => return Err("usage: nomoc build <source.nomo> [--emit-c] [--out path]".to_string()),
        }
    }
    source
        .map(|source| (source, out))
        .ok_or_else(|| "usage: nomoc build <source.nomo> [--emit-c] [--out path]".to_string())
}

fn print_help() {
    println!(
        "nomoc 0.1.0\n\nCommands:\n  nomoc check <source.nomo> [--json-errors]\n  nomoc build <source.nomo> [--emit-c] [--out path]\n"
    );
}
